CREATE DATABASE IF NOT EXISTS dz_4 ;
...
(SELECT shopname
FROM shops
WHERE cats.shops_id = shops.id) as shops
FROM cats;
-- ещё вариант 
...
from shops
where id NOT in 
(select shops_id from cats where name = 'Murzik' or name = 'Zuza');
...
select ord_datetime as data ,an_name as name, an_price as price
from Analysis
